...
const validateTx = async () => {
    const isProcessRunningDesc = await finuxTxProcesses.findOne({type: "chain"});
    ...
    if(!isProcessRunning) {
        ...
        const chainTxs = await finuxTxs.find({transferType: "chain", sent: true, validated: false, txCompleted: false});
                            ...
                            if(pollReqKey.data !== undefined && pollReqKey.data !== null) {
                                ...
                                if(resKeys.includes(chainTxs[i]["requestKey"])) {
                                    ...
                                    if(result === "success") {
                                        ...
                                        await finuxTxs.updateOne(
                                            {_id: chainTxs[i]["_id"]},
                                            {$set: {validated: true, validatedTimestamp: nowUnix, txOutcome: "success", txCompleted: true}}
                                        );
