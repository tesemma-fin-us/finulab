...
const closeMarketValidate = async () => {
    ...
    if(!isProcessRunning) {
        ...
        const marketsToValidate = await predictionsResolutionDesc.find({closeSent: true, closeValidated: false});
        if(marketsToValidate.length > 0) {
            for(let i = 0; i < marketsToValidate.length; i++) {
                if(marketsToValidate[i]["closeValidationAttempts"] >= 10) {
                    ...
                    try {
                        const pollReqKey = await axios.post(`${chainwebSendURLpt1}${chainId}${chainwebSendURLpt2}`, 
                            {
                                "requestKeys": [
                                    marketsToValidate[i]["closeRequestKey"]
                                ]
                            }
                        );
                        ...
                        if(pollReqKey.status === 200) {
                            if(pollReqKey.data !== undefined && pollReqKey.data !== null) {
                                ...
                                if(resKeys.includes(marketsToValidate[i]["closeRequestKey"])) {
                                    const result = pollReqKey.data[marketsToValidate[i]["closeRequestKey"]]["result"]["status"];
                                    ...
                                    if(result === "success") {
                                        ...
                                        await predictionsResolutionDesc.updateOne(
                                            {_id: marketsToValidate[i]["_id"]},
                                            {$set: {closeValidated: true, closeValidatedTimestamp: nowUnix}}
                                        );
