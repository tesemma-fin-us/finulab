...
const closeMarket = async () => {
    ...
    if(!isProcessRunning) {
        ...
        const marketsToClose = await predictionsResolutionDesc.find({readyForClose: true, closeSent: false});
        if(marketsToClose.length > 0) {
            ...
            for(let i = 0; i < marketsToClose.length; i++) {
                const now = new Date();
                const creationTime = datefns.getUnixTime(now);
                const chainId = String(marketsToClose[i]["chainId"]);

                const pactCmd = pact.simple.exec.createCommand(capabilityData, undefined,
                    `(free.finux.y-n-close-market "${marketsToClose[i]["marketId"]}")`,
                ...
                if(utilizeRequestKey !== "") {
                    await predictionsResolutionDesc.updateOne(
                        {_id: marketsToClose[i]["_id"]}, 
                        {$set: {closeSent: true, closeRequestKey: utilizeRequestKey, closeSentTimestamp: creationTime}}
                    );
                }
            }
        }
